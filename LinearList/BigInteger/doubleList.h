...
protected:
    ...
    int length;
public:
    doubleList(const T* data = nullptr, int n = 0);        //构造函数
    ...
    void insert(T value, dN* node);              //在node后插入节点
    T Delete(int i);                    //删除位置为i的节点
    T Delete(dN* node);                 //删除node节点
    vector<int> Find(T target);         //找到值为target的节点，并返回其坐标vector
    int getLength()const;                    //获取链表长度
    void show();                        //显示链表内容
    void push_back(T value);            //在链表尾加上值为value的节点
    void push_back(dN* node);           //在链表尾加上节点node
    ~doubleList();                      //析构函数
